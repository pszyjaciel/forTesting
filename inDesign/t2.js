   /**
	 * Lets play with colors to use colors you NEED to create a swatch
	 */
    ...
    // make a document
    var pw = 100;
    var ph = 34;	// wysokosc prostokonta
    ...
    alert("the swatch with the number "+ random_num +" is always\n[" + random_swatch.name + "]");
        
    // now lets create some colors
    var cols = new Array();
    var num_of_colors = 10;
    // lets do it randomly
    $.writeln('num_of_colors: ' + num_of_colors);
    ...
    var step = pw / cols.length; // calc the number of steps we have. one by
									// color
    // coordinates
    var x1 = 0;
    var y1 = 0;
    ...
    // add CMYK color
    // myColorAdd(app.activeDocument, "My Custom Color", ColorModel.PROCESS,
	// [80,50,30,10]);
    ...
    // app.selection[0].fillColor = myColorAdd(app.activeDocument, "My Custom
	// Color", ColorModel.PROCESS, [80,50,30,10]);
    
    function color_add(myDocument, myColorName, myColorModel, myColorValue){
        if(myColorValue instanceof Array == false){
            myColorValue = [(parseInt(myColorValue, 16) >> 16 ) & 0xff, (parseInt(myColorValue, 16) >> 8 ) & 0xff,    
